...
# length(unique(df.NWIS.TN_CQ$site_no))  88 sites
...
length(unique(df.NWIS.TN_CQ$site_no))
...
temp1<-df.NWIS.TN_CQ%>%filter(sample_dt >= as.Date('2001-01-01'))%>%
  group_by(site_no)%>%
  mutate(n=n())%>%
  filter(n>=20)
...
temp2<-temp1%>%filter(site_no %in% filter(df.NWIS.TN_site_metadata, dec_lat_va >40.9364)$site_no)
...
x1<-filter(df.NWIS.TN_CQ, site_no %in%df.G2$STAID)
...
x2<-x1%>%filter(site_no %in% filter(df.NWIS.TN_site_metadata, dec_lat_va >40.9364)$site_no)
...
df.3 <- df.TN_CQ %>% filter(site_no %in% df.Q.range$site_no)

length(unique(df.3$site_no))

# save site lists for df.2 and df.3:

save(df.2, file = 'Processed_Data/TN.df.2.sites.Rdata')
save(df.3, file = 'Processed_Data/TN.df.3.sites.Rdata')
...
l_Seg<-lapply(seq_along(unique(df.TN_CQ$site_no)), \(i) fun.CQ.BP(i, df.TN_CQ))
...
df.Q<-df.NWIS.Q.for_TN%>% # this gives df of complete hydrographs for each site and includes columns to group by calender day later on
...
l.DA<-df.NWIS.TN_site_metadata %>% filter(site_no %in% names(l.Q))%>%select(site_no, drain_area_va)%>%split(., .$site_no)%>%lapply(., \(i) i$drain_area_va)
