...
# arrange by number of SRP observations. To do this:
...
# merge this df with df.NWIS.SRP_CQ and arrange by the new column:
...
# build a matrix of the number of SRP samples as a function of watershed size. To do this:
...
temp1<-df.NWIS.SRP_CQ%>%filter(sample_dt >= as.Date('2001-01-10'))
...
temp2<-filter(df.NWIS.SRP_site_metadata, dec_lat_va >40.9364)

# now use this site list to filter down the df.SRP_CQ:
...
df.SRP_CQ<-temp4%>%filter(sample_dt >= as.Date('2001-01-10'))
...
x<-filter(df.NWIS.SRP_site_metadata, site_no %in% df.G2$STAID)
...
# save(df.SRP_CQ,file = 'Processed_Data/SRP.Rdata')
...
df.SRP_CQ_for_BP<-df.SRP_CQ%>%
  drop_na(result_va, Q_yield)

# create a vector of ordered unique site names:

temp.loop<-sort(unique(df.SRP_CQ_for_BP$site_no))
