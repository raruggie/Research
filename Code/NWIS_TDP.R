...
# use function I created (sourced) to get a dataframe of sites for just TDP with #samples = 20 threshold:

# df.NWIS.TDP_sites<-fun.df.Pair_consit_flow(pcode, df.NWIS.Q_sites, n_samples = 20, state = 'NY')

# write.csv(df.NWIS.TDP_sites, "Raw_Data/df.NWIS.TDP_sites.csv", row.names=FALSE)

df.NWIS.TDP_sites<-read.csv("Raw_Data/df.NWIS.TDP_sites.csv", colClasses = c(site_no = "character"))
...
# which ever sites do notoverlap with TDP will be downloaded here. To do this:
...
# determine which of the TDP sites dont overlap:

# df.overlap<-df.NWIS.TDP_sites%>%filter(!site_no %in% df.NWIS.Q.combined$site_no)

# download the raw flow data for the sites still needed:

# df.NWIS.Q.TDP_needed<-readNWISdv(siteNumbers = df.overlap$site_no, parameterCd = '00060', startDate = "", endDate = "", statCd = "00003")

# returns no flow data for the 7 overlap

# rename flow to 'df.NWIS.Q.for_TDP':

df.NWIS.Q.for_TDP<-df.NWIS.Q.combined; rm(df.NWIS.Q.combined)

# filter to just the TDP sites:

df.NWIS.Q.for_TDP<-df.NWIS.Q.for_TDP%>%filter(site_no %in% df.NWIS.TDP_sites$site_no)

# download the raw discrete TDP sample data:

# df.NWIS.TDP<-readNWISqw(siteNumbers = df.NWIS.TDP_sites$site_no, parameterCd = pcode)

# write.csv(df.NWIS.TDP, "Raw_Data/df.NWIS.TDP.csv", row.names=FALSE)

df.NWIS.TDP<-read.csv("Raw_Data/df.NWIS.TDP.csv", colClasses = c(site_no = "character"))%>%mutate(sample_dt = as.Date(sample_dt))
...
# join the TDP with the flow df:

df.NWIS.TDP_CQ<-left_join(df.NWIS.TDP, df.NWIS.Q.for_TDP, by=c("site_no"="site_no", "sample_dt"="Date"))

# remove observations where there are not CQ pairs:

df.NWIS.TDP_CQ<-df.NWIS.TDP_CQ%>%drop_na(X_00060_00003)

# take average of multiple samples on the same day at the same site:

df.NWIS.TDP_CQ<-df.NWIS.TDP_CQ%>%
  group_by(site_no, sample_dt)%>%
  summarise_at(vars(result_va, X_00060_00003), funs(mean(., na.rm=TRUE)))

# arrange by number of TDP observations. To do this:

# first arrange the dataframe with the number of samples: 

temp<-df.NWIS.TDP%>%group_by(site_no)%>%
  summarise(n=n())%>%
  arrange(desc(n))%>%
  mutate(n_sample_rank=rank(-n, ties.method='first'))

# merge this df with df.NWIS.TDP_CQ and arrange by the new column:

df.NWIS.TDP_CQ<-left_join(df.NWIS.TDP_CQ,temp, by='site_no')%>%
...
# df.NWIS.TDP_site_metadata<-readNWISsite(siteNumbers = unique(df.NWIS.TDP_CQ$site_no))

# write.csv(df.NWIS.TDP_site_metadata, "Raw_Data/df.NWIS.TDP_site_metadata.csv", row.names=FALSE)

df.NWIS.TDP_site_metadata<-read.csv("Raw_Data/df.NWIS.TDP_site_metadata.csv", colClasses = c(site_no = "character"))

# then select just the site number and DA column:

df.DA<-df.NWIS.TDP_site_metadata%>%
  select(site_no, drain_area_va)

# finally merge with df.NWIS.TDP_CQ and create a new Q column with area normalized flows (not worrying about units right now): 
...
df.NWIS.TDP_CQ<-left_join(df.NWIS.TDP_CQ, df.DA, by = 'site_no')%>%
...
# build a matrix of the number of TDP samples as a function of watershed size. To do this:

# I already have a df with paired CQ observations and DA, just need to get the distinct sites:

TDP_sites<-df.NWIS.TDP_CQ%>%distinct(site_no, .keep_all = T)
...
for (i in seq_along(n_sam)){
  ...
  temp.i<-TDP_sites%>%filter(n>n_sam[i])
...
temp2<-filter(df.NWIS.TDP_site_metadata, dec_lat_va >40.9364)

# now use this site list to filter down the df.TDP_CQ:
...
x<-filter(df.NWIS.TDP_site_metadata, site_no %in% df.G2$STAID)
...
df.TDP_CQ_for_BP<-df.TDP_CQ%>%
  drop_na(result_va, Q_yield)

# create a vector of ordered unique site names:

temp.loop<-sort(unique(df.TDP_CQ_for_BP$site_no))
...
# map.NWIS.TDP_sites<-df.NWIS.TDP_site_metadata%>%
...
l.TDP_CQ.lm<-lapply(l.TDP_CQ, \(i) lm(log_C~log_Q, data=i))

# save the model coef ad pvals:

TDP_CQ.coef<-as.data.frame(bind_rows(lapply(l.TDP_CQ.lm, \(i) summary(i)$coefficients[,1]), .id = 'site_no'))%>%
  rename(Intercept = 2, Slope = 3)

# save the pvalues 

TDP_CQ.pvals<-as.data.frame(bind_rows(lapply(l.TDP_CQ.lm, \(i) summary(i)$coefficients[,4]), .id = 'site_no'))%>%
  rename(Intercept.pval = 2, Slope.pval = 3)

# merge the two dfs:

df.lm<-left_join(TDP_CQ.coef,TDP_CQ.pvals,by='site_no')
...
df.TDP_CQ<-left_join(df.TDP_CQ,df.lm%>%select(site_no, Type),by=c('Name'='site_no'))
